...
function AlbumCreator(props) {
    ...
    const button = {
        "invalid": {onClick: undefined, text: "Create Album", classes: "is-warning"},
        "valid": {onClick: createAlbum, text: "Create Album", classes: "is-primary"},
        "loading": {onClick: createAlbum, text: "", classes: "is-loading"},
        "done": {onClick: props.close, text: "Close", classes: "is-success"}
    }[mode];
    ...
    function createAlbum() {
        ...
        PhotosAPI.createAlbum(album.name, album.description)
            .then(res => { if (!res.ok) {console.log("Response status: " + res.status)}})
            .then(() => setMode("done"))
    ...
    useEffect(() => {
        let valid = true;
        if (album.name.length < 3 || existingAlbums.indexOf(album.name) !== -1) {
            valid = false;
        }
                ...
                <section className="modal-card-body">
                    <div className="field">
                        ...
                        <div className="control has-icons-right">
                            <input className="input" type="text" placeholder="Album name" value={album.name}
                                   onChange={(e) => setAlbum(updated(album, "name", e.target.value))}/>
                            <span className="icon is-small is-right">
                                <i className="fas fa-check"/>
                            </span>
                        </div>
                        <p className="help is-hidden">This username is available</p>
                    </div>
                    <div className="field">
                        <label className="label">Description</label>
                        <div className="control">
                            <textarea className="textarea" placeholder="Description"
                                      onChange={(e) => setAlbum(updated(album, "description", e.target.value))}/>
                        </div>
                    </div>
                </section>
